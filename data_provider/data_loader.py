...
class Dataset_ETT_hour(Dataset):
    ...
    def __len__(self):
        b=  len(self.data_x)
        a = len(self.data_x) - self.seq_len - self.pred_len + 1 
...
class Dataset_ETT_minute(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 ...
                 target='OT', scale=True, timeenc=0, freq='t', seasonal_patterns=None): #t:minutely
        # size [seq_len, label_len, pred_len]
        # info
        if size == None:
            self.seq_len = 24 * 4 * 4
...
class Dataset_UK_DALE_minute(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 features='S', data_path='ETTm1.csv',subsequence_num=4,
                 target='OT', scale=True, timeenc=0, freq='t', seasonal_patterns=None,interval=1,model_id =' ',decomposition_method ='ssa'): #t:minutely
    ...
    def __preprocessing_SSA(self, data):  
        result_ssa = SSA(data.reshape(-1, ), self.subsequence_num)  
        return result_ssa 
    ...
    def __read_data__(self):
        ...
        train_num = 8400//self.interval
        test_num =1200 //self.interval
        origin_dataset_len = train_num+test_num+self.seq_len

     
        data_itved = [] 

        cols_data_value = df_raw.columns[1:]
        df_data_value = df_raw[cols_data_value].values
        df_data_value = df_data_value[1:]

        for i in range(len(df_raw)):
            data_itved.append(sum(df_data_value[i * self.interval:(i + 1) * self.interval]))

        df_raw_type = data_itved[0:origin_dataset_len]

        if self.scale:
            train_data = df_raw_type
            self.scaler.fit(train_data)
            data = self.scaler.transform(train_data)
            self.raw_data =df_raw_type 
            self.scaler_data =data
        else:
            data = data_itved

  
        if (self.decomposition_method == "ssa"):
            data_SSA = self.__preprocessing_SSA(data)  
            data_SSA = data_SSA.T

        if (self.decomposition_method == "vmd"): 
            data_SSA = self.__preprocessing_VMD(data)
            data_SSA = np.array(data_SSA).reshape(4, -1).T


        if (self.decomposition_method == "swt"): 
            data_SSA = self.__preprocessing_SWT(data)
            data_SSA = np.array(data_SSA).reshape(4, -1).T

        if (self.decomposition_method == "ewt"): 
            data_SSA = self.__preprocessing_EWT(data)
            data_SSA = np.array(data_SSA).reshape(4, -1).T

        if (self.decomposition_method == "emd"):
            data_SSA = self.__preprocessing_EMD(data)
            data_SSA = np.array(data_SSA).reshape(2, -1).T

        border1s = [0,                              train_num]
        border2s = [train_num+self.seq_len,origin_dataset_len]

        border1 = border1s[self.set_type]
        border2 = border2s[self.set_type]

        df_stamp = df_raw.iloc[::self.interval][['date']][border1:border2]
        ...
        self.data_x = data_SSA[border1:border2]  
        self.data_y = data_SSA[border1:border2]
    ...
    def __getitem__(self, index):  
        ...
        seq_x_raw = self.raw_data[s_begin:s_end]
        seq_y_raw= self.raw_data[r_begin:r_end]

        seq_y_label = self.raw_data[index + self.seq_len]  

        seq_x_mark = self.data_stamp[s_begin:s_end]
        seq_y_mark = self.data_stamp[r_begin:r_end]


        return seq_x, seq_y, seq_x_mark, seq_y_mark,seq_y_label
